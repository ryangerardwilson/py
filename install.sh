...
print_manual_shell_steps() {
  ...
  print_message info "Manually add to ~/.bashrc: [ -r \"$APP_DIR/$APP/shell/py.bash\" ] && source \"$APP_DIR/$APP/shell/py.bash\""
  printed=true
...
if [[ ! -f "$tmp_dir/${APP}/shell/py.bash" ]]; then
  die "Archive did not contain expected shell hook 'shell/py.bash'"
fi
